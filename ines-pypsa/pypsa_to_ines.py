...
def main():
    ...
    with api.DatabaseMapping(url_db_in) as source_db:
        with api.DatabaseMapping(url_db_out) as target_db:
            ...
            target_db = ines_transform.transform_parameters_entity_from_parameter(source_db, target_db, parameters_to_parameters, ts_to_map=True)   
...
def add_time_structure(source_db,target_db):
    alt_ent_class_source = ['PyPSA', ('Time',) ,'Network']
    #timeline
    ...
    target_db = ines_transform.add_item_to_DB(target_db, 'timeline', ["PyPSA", ('Time',), 'system'], snapshots, value_type=True)
    #resolution
    #This is just the difference between the first two timesteps ie. assumes constant resolution
    minutes_diff = (datetime.fromisoformat(snapshots.values[1])-datetime.fromisoformat(snapshots.values[0])).total_seconds() / 60.0
    ines_transform.assert_success(target_db.add_entity_item(entity_class_name='temporality',entity_byname=('resolution',)), warn=True)
    target_db = ines_transform.add_item_to_DB(target_db, 'resolution', ["PyPSA", ('resolution',), 'temporality'], minutes_diff, value_type=True)
    ...
    if periods:
        target_db = ines_transform.add_item_to_DB(target_db, 'period', ["PyPSA", ('solve',), 'solve_pattern'], periods, value_type=True)
    ...
    for i in investment_period_weightings.to_dict()["data"]:
        if i[0] == 'years' and i[1] != None:
            for j in i[1].to_dict():
                ines_transform.assert_success(target_db.add_entity_item(entity_class_name='period',entity_byname=(j[0],)), warn=True)
                target_db = ines_transform.add_item_to_DB(target_db, 'years_represented', ["PyPSA", (j[0],), 'period'], j[1], value_type=True)
...
def create_market_nodes(source_db, target_db):
    ...
    for source_entity in source_db.get_entity_items(entity_class_name='Carrier'): 
        alt_ent_class_source = alt_ent_class_source = ["PyPSA", source_entity["entity_byname"],'Carrier']
        ...
        if isinstance(co2_emissions, float):
            if co2_emissions > 0.0:
                ...
                target_db = ines_transform.add_item_to_DB(target_db, 'co2_content', ["PyPSA", market_byname, 'node'], 
                                                          co2_emissions, value_type=True)
...
def create_market_relationships(source_db, target_db, market_carrier_list):
    for source_entity in source_db.get_entity_items(entity_class_name='Generator'): 
        alt_ent_class_source = alt_ent_class_source = ["PyPSA", source_entity["entity_byname"],'Generator']
...
def add_carrier_investment_limits(source_db,target_db):
    periods = ines_transform.get_parameter_from_DB(source_db,"investment_periods", ['PyPSA', ('Time',) ,'Network'])
    for source_entity in source_db.get_entity_items(entity_class_name='Carrier'):
        alt_ent_class_source = alt_ent_class_source = ["PyPSA", source_entity["entity_byname"],'Carrier']
        ...
        target_db = ines_transform.add_item_to_DB(target_db, 'invest_max_period', ["PyPSA",(source_entity["name"],),'set'],value=periodic_limit, value_type=True)    
...
def map_links_to_units(source_db,target_db):
    ...
    for source_entity in source_db.get_entity_items(entity_class_name='Link'): 
        alt_ent_class_source = ["PyPSA", source_entity["entity_byname"],'Link']
        ...
        unit1_name = source_entity["name"] +"_1"
        ...
        investment_binding_map = {
            "1": (["PyPSA", unit1_byname, 'unit'], -1),
        }

        #check if the other direction unit is needed
        neg_flag = False
        if isinstance(p_min_pu, api.Map):
            new_values = []
        ...
        if neg_flag:  #allows flow for both directions
            unit2_name = source_entity["name"] +"_2"
            ...
            investment_binding_map["2"] = (["PyPSA", unit2_byname, 'unit'],1)
            if p_nom_extendable:
                target_db = bind_investments(source_db, target_db, alt_ent_class_source, ["PyPSA", out_entity_byname, 'unit__to_node'], investment_binding_map)
    return target_db
...
def create_link_unit_params(source_db, target_db, names, alt_ent_class_source, p_min_pu = None):
    ...
    for name, target_name in parameters_dict.items(): 
        ...
        target_db = ines_transform.add_item_to_DB(target_db, target_name, ["PyPSA", unit_byname, 'unit'], value, value_type=True)
    ...
    if committable:
        ...
        value= 'no_startup'
    target_db = ines_transform.add_item_to_DB(target_db, 'startup_method', ["PyPSA",unit_byname,'unit'],value=value, value_type=True)
    ...
    for name, target_name in parameters_dict.items():
        value = ines_transform.get_parameter_from_DB(source_db,name, alt_ent_class_source)
        target_db = ines_transform.add_item_to_DB(target_db, target_name, ["PyPSA", out_entity_byname, out_class], value, value_type=True)
        target_db = ines_transform.add_item_to_DB(target_db, target_name, ["PyPSA", in_entity_byname, in_class], value, value_type=True)
    
    if p_min_pu != None:
        if isinstance(p_min_pu, api.Map) or (isinstance(p_min_pu, float) and p_min_pu != 1.0):
            target_db = ines_transform.add_item_to_DB(target_db, 'profile_limit_upper', ["PyPSA", out_entity_byname, out_class], p_min_pu, value_type=True)
    else:
        value = ines_transform.get_parameter_from_DB(source_db, "p_max_pu", alt_ent_class_source)
        if isinstance(value, api.Map) or (isinstance(value, float) and value != 1.0):
            target_db = ines_transform.add_item_to_DB(target_db, 'profile_limit_upper', ["PyPSA", out_entity_byname, out_class], value, value_type=True)

    relationship_list = [["PyPSA", out_entity_byname, out_class],["PyPSA", in_entity_byname, in_class]]
    target_db = add_unit_capacities(source_db,target_db, alt_ent_class_source, ["PyPSA", unit_byname, 'unit'],relationship_list)
    target_db = add_lifetime(source_db, target_db, alt_ent_class_source, ["PyPSA", unit_byname, 'unit'])
...
def map_loads_to_nodes(source_db,target_db):
    ...
    for source_entity in source_db.get_entity_items(entity_class_name='Load'): 
        alt_ent_class_source = ["PyPSA", source_entity["entity_byname"],'Load']
        ...
        alt_ent_class_target = ["PyPSA", (bus,) ,'node']

        if isinstance(p_set_param, api.Map):
            new_values = []
...
def map_storageUnits_to_nodes_and_units(source_db,target_db):
    for source_entity in source_db.get_entity_items(entity_class_name='StorageUnit'): 
        alt_ent_class_source = ["PyPSA", source_entity["entity_byname"],'StorageUnit']
        ...
        target_db = ines_transform.add_item_to_DB(target_db, 'node_type', ["PyPSA", node_byname, 'node'], 'storage', value_type=True)
        node_param_dict = {
            'inflow': 'flow_profile',
        ...
        for name, target_name in node_param_dict.items():
            value = ines_transform.get_parameter_from_DB(source_db, name, alt_ent_class_source)
            target_db = ines_transform.add_item_to_DB(target_db, target_name, ["PyPSA", node_byname, 'node'], value, value_type=True)
        
        target_db = add_storage_capacities(source_db, target_db, alt_ent_class_source, ["PyPSA", node_byname, 'node'])
        target_db = add_lifetime(source_db, target_db, alt_ent_class_source, ["PyPSA", node_byname, 'node'], storage= True) 
        ...
        investment_binding_map = {
            "storage" : (["PyPSA", node_byname, 'node'], -1/max_hours),
            "out": (["PyPSA", unit1_byname, 'unit'], 1),
        }
        ...
        if neg_flag:
            ...
            investment_binding_map["in"] = (["PyPSA",unit2_byname, 'unit'],1)

        if p_nom_extendable:
            target_db = bind_investments(source_db, target_db, alt_ent_class_source, ["PyPSA", node_byname, 'node'], investment_binding_map, storage= True)
...
def create_storageUnit_params(source_db, target_db, names, alt_ent_class_source, p_min_pu = None):
    ...
    in_class = names[3]
    in_entity_byname = names[4]
    relationship_list = [["PyPSA", out_entity_byname, out_class]]
    ines_transform.assert_success(target_db.add_entity_item(entity_class_name=out_class,entity_byname=out_entity_byname,
                ), warn=True)
    ines_transform.assert_success(target_db.add_entity_item(entity_class_name=in_class,entity_byname=in_entity_byname,
                ), warn=True)
    ...
    if p_min_pu != None:
        ...
        target_db = ines_transform.add_item_to_DB(target_db, 'efficiency', ["PyPSA", unit_byname, 'unit'], value, value_type=True)
        relationship_list.append(["PyPSA", in_entity_byname, in_class]) 
...
def add_generator_modified_parameters(source_db, target_db):
    for source_entity in source_db.get_entity_items(entity_class_name='Generator'):
        alt_ent_class_source = ["PyPSA", source_entity["entity_byname"],'Generator']
        alt_ent_class_unit = ["PyPSA", (source_entity["name"],),'unit']

        bus = ines_transform.get_parameter_from_DB(source_db, 'bus', alt_ent_class_source)
        alt_ent_class_relationship = ["PyPSA", (source_entity["name"],bus),'unit__to_node']
        ...
        if isinstance(upper_limit, api.Map) or (isinstance(upper_limit, float) and upper_limit != 1.0):
        ...
        if isinstance(lower_limit, api.Map) or (isinstance(lower_limit, float) and lower_limit > 0.0):
...
def add_line_capacities_and_lifetimes(source_db, target_db):
    for source_entity in source_db.get_entity_items(entity_class_name='Line'):
        alt_ent_class_source = ["PyPSA", source_entity["entity_byname"],'Line']
        alt_ent_class_target = ["PyPSA", (source_entity["name"],),'link']
...
def add_store_capacities_and_lifetimes(source_db, target_db):
    for source_entity in source_db.get_entity_items(entity_class_name='Store'):
        alt_ent_class_source = ["PyPSA", source_entity["entity_byname"],'Store']
        alt_ent_class_target = ["PyPSA", (source_entity["name"],),'node']
...
def bind_investments(source_db, target_db, alt_ent_class_source, alt_ent_class_cost, investment_binding_map, storage = False):
    capital_cost = ines_transform.get_parameter_from_DB(source_db, 'capital_cost', alt_ent_class_source)
    target_param = 'investment_cost'
    if storage:
        target_param = 'storage_investment_cost'
        name_list =[]
        directions = []
        ...
        for direction in directions:
            ...
            alt_ent_class_constraint = ["PyPSA",(constraint_name,),'constraint']
            ines_transform.assert_success(target_db.add_entity_item(
                                entity_class_name="constraint",
                                entity_byname=(constraint_name,),
                            ), warn=True)
            ines_transform.add_item_to_DB(target_db, "constant", alt_ent_class_constraint, value=0, value_type=True)
            ines_transform.add_item_to_DB(target_db, "sense", alt_ent_class_constraint, value="equal", value_type=True)
            for key, data in investment_binding_map.items():
                if key == direction:
                    output_map = api.Map([constraint_name], [data[1]], index_name="constraint")
                    ines_transform.add_item_to_DB(target_db, "constraint_unit_count_coefficient", data[0], value = output_map, value_type=True)
        for key, data in investment_binding_map.items():
            if key == "storage":
                output_map = api.Map(name_list, [data[1] for i in name_list], index_name="constraint")
                ines_transform.add_item_to_DB(target_db, "constraint_storage_count_coefficient", data[0], value = output_map, value_type=True)
                capital_cost = capital_cost * (-data[1])
    else:
    ...
    ines_transform.add_item_to_DB(target_db, target_param, alt_ent_class_cost, value=capital_cost, value_type=True)
    return target_db
...
def add_profile_methods(target_db):
    for classes in ["unit__to_node","node__to_unit"]:
        for source_entity in target_db.get_entity_items(entity_class_name=classes):
            alt_ent_class_target = ["PyPSA", source_entity["entity_byname"], classes]
...
def add_inflow_methods_and_state_fix(target_db):
    for target_entity in target_db.get_entity_items(entity_class_name='node'): 
        alt_ent_class_source = ["PyPSA", target_entity["entity_byname"], 'node']
        ...
        if isinstance(flow_profile,api.Map):
        ...
        upper = False
        lower = False
        if isinstance(fix_limit, api.Map) or (isinstance(fix_limit, float) and fix_limit != 1.0):
            fix = True
        if isinstance(upper_limit, api.Map) or (isinstance(upper_limit, float) and upper_limit != 1.0):
            upper = True
        if isinstance(lower_limit, api.Map) or (isinstance(lower_limit, float) and lower_limit != 1.0):
            lower = True
        if fix:
...
if __name__ == "__main__":
    ...
    if developer_mode:
        ...
        url_db_in ='sqlite:///C:/Users/aetart/Documents/ines-pypsa/test_workflow/pypsa_db_test.sqlite'
        url_db_out = 'sqlite:///C:/Users/aetart/Documents/ines-pypsa/test_workflow/ines-spec_test.sqlite'
        # open yaml files
